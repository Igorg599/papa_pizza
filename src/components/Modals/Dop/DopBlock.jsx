...
function DopBlock({item}) {
    ...
    return (
        ...
        className={activeIcon ? "items-block__dop-button active" : "items-block__dop-button"}>
            <img src={item.img} className="items-block__dop-img" alt="img"></img>
            <h4 className="items-block__dop-title">{item.name}</h4>
            <span className="items-block__dop-money"><span className="money__value">{item.price}</span><span className="money__currency money__currency_on-the-right"> ₽</span></span>
